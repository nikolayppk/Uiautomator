...
//Кейс авторизации по токену вторичной авторизации
public class UC_ST1_AC1 {
    ...
    public void Test() throws InterruptedException, RemoteException {
        ...
        closeApp();//закрываем полностью приложение
        Thread.sleep(3000);
        support.RunApp();//запускаем приложение
        toolbar();//вызываем шторку
        checkID();//чекаем id после вторичной авторизации
    ...
    private void toolbar() throws InterruptedException {
        ...
        support.TrySearch_by_id(toolbar.search_icon, 15000);
